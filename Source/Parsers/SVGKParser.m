...
- (SVGKParseResult*) parseSynchronously
{
	...
	{
		NSLog(@"FATAL: attempting to run the parser twice in one thread; limxml is single-threaded only, so we are too (until someone wraps libxml to be multi-threaded)");
	}
	...
	*/
	...
	if( ctx ) // if libxml init succeeds...
	{
		...
		while( bytesRead > 0 )
		{
			...
			{
				NSLog( @"SVGKParser: 'cancel parse' discovered; bailing on this XML parse" );
				break;
			}
			else
			{
			...
			@catch( NSException* e )
			{
				NSLog( @"Exception while trying to parse SVG file, will store in parse results. Exception = %@", e);
			...
			if( [currentParseRun.errorsFatal count] > 0 )
			{
				...
				{
				NSLog(@"[%@] libXml reported internal parser error with magic libxml code = %li (look this up on http://xmlsoft.org/html/libxml-xmlerror.html#xmlParserErrors)", [self class], (long)libXmlParserParseError );
...
- (SVGKSource *)loadCSSFrom:(NSString *)href
{
    ...
    if( cssSource == nil )
    {
        ...
        if( cssText == nil )
        {
            NSLog(@"[%@] Unable to find external CSS file '%@'", [self class], href );
        }
        else
        {
