...
+ (CGPoint) readMovetoDrawtoCommandGroups:(NSScanner*)scanner path:(CGMutablePathRef)path relativeTo:(CGPoint)origin isRelative:(BOOL) isRelative
{
	...
	NSLog(@"Parsing command string: move-to, draw-to command");
#endif
...
{
#if VERBOSE_PARSE_SVG_COMMAND_STRINGS
	NSLog(@"Parsing command string: line-to command");
#endif
...
{
#if VERBOSE_PARSE_SVG_COMMAND_STRINGS
	NSLog(@"Parsing command string: quadratic-bezier-curve-to command");
#endif
...
#if DEBUG_PATH_CREATION
	NSLog(@"[%@] PATH: QUADRATIC CURVE to (%2.2f, %2.2f)..(%2.2f, %2.2f)", [SVGKPointsAndPathsParser class], curveResult.c1.x, curveResult.c1.y, curveResult.p.x, curveResult.p.y);
#endif
...
{
#if VERBOSE_PARSE_SVG_COMMAND_STRINGS
	NSLog(@"Parsing command string: smooth-quadratic-bezier-curve-to command");
#endif
	NSString* cmd = nil;
...
#if DEBUG_PATH_CREATION
	NSLog(@"[%@] PATH: SMOOTH QUADRATIC CURVE to (%2.2f, %2.2f)..(%2.2f, %2.2f)", [SVGKPointsAndPathsParser class], thisCurve.c1.x, thisCurve.c1.y, thisCurve.p.x, thisCurve.p.y );
#endif
...
{
#if VERBOSE_PARSE_SVG_COMMAND_STRINGS
	NSLog(@"Parsing command string: curve-to command");
#endif
    NSString* cmd = nil;
...
#if DEBUG_PATH_CREATION
	NSLog(@"[%@] PATH: CURVE to (%2.2f, %2.2f)..(%2.2f, %2.2f)..(%2.2f, %2.2f)", [SVGKPointsAndPathsParser class], curveResult.c1.x, curveResult.c1.y, curveResult.c2.x, curveResult.c2.y, curveResult.p.x, curveResult.p.y);
#endif
...
{
#if VERBOSE_PARSE_SVG_COMMAND_STRINGS
	NSLog(@"Parsing command string: vertical-line-to command");
#endif
    NSString* cmd = nil;
...
{
#if VERBOSE_PARSE_SVG_COMMAND_STRINGS
	NSLog(@"Parsing command string: horizontal-line-to command");
#endif
    NSString* cmd = nil;
...
{
#if VERBOSE_PARSE_SVG_COMMAND_STRINGS
	NSLog(@"Parsing command string: close command");
#endif
    NSString* cmd = nil;
